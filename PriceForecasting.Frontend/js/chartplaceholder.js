document.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadRecommendations() {
    try {
      const res = await fetch(
        `priceforecastingsystem.railway.internal/api/recommendations/${article}?period=${currentPeriod}&scenario=${scenarioMap[currentScenario]}`
      );
  ...
  async function loadForecast() {
    try {
      const res = await fetch(
        `priceforecastingsystem.railway.internal/api/forecast/${article}?days=${currentPeriod}`
      );
